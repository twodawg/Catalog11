...
namespace Catalog1.Models
{
    public class Repository
    {
        ...
        {
            var cart = GetCartByName(username).First();

            EmailReceipt(username, cart);

            cart.IsFulfilled = true;

            if (db.Entry(cart) != null)
                db.Entry(cart).State = System.Data.Entity.EntityState.Modified;

            db.SaveChanges();
        }

        private void EmailReceipt(string username, Cart cart)
        {
            string orderitems = "";

            foreach(CartItem cartitem in cart.Carts)
            {
            ...
            var email = new Email
            {
                ...
                Content = "Here is what you ordered,\n" + orderitems,
            };
            db.Emails.Add(email);
            db.SaveChanges();
        }
